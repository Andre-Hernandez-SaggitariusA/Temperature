...
// Compilar:
// g++ -O3 -shared -std=c++11 -fPIC $(python3 -m pybind11 --includes) -fopenmp calor.cpp -o propagacion$(python3-config --extension-suffix)
...
void actualizar(int Nx,
		...
		py::array_t<double> u_old_py,
		py::array_t<double> u_new_py,
		int hilos){
		
	py::buffer_info u_new_b = u_new_py.request();
	py::buffer_info u_old_b = u_old_py.request();
	...
	double *u_old = static_cast<double *>(u_old_b.ptr);
	...
	for(int j = 1; j < Ny; j++){
		for(int i = 1; i < Nx; i++){
			...
			u_new[0*s_k + j*s_j + i] = pa*u[0*s_k + j*s_j + i] - pv*u_old[0*s_k + j*s_j + i]
			...
			u_new[Nz*s_k + j*s_j + i] = pa*u[Nz*s_k + j*s_j + i] - pv*u_old[Nz*s_k + j*s_j + i]
	...
	for(int k = 1; k < Nz; k++){
		for(int i = 1; i < Nx; i++){
			...
			u_new[k*s_k + 0*s_j + i] = pa*u[k*s_k + 0*s_j + i] - pv*u_old[k*s_k + 0*s_j + i]
			...
			u_new[k*s_k + Ny*s_j + i] = pa*u[k*s_k + Ny*s_j + i] - pv*u_old[k*s_k + Ny*s_j + i]
	...
	for(int k = 1; k < Nz; k++){
		for(int j = 1; j < Ny; j++){
			...
			u_new[k*s_k + j*s_j + 0] = pa*u[k*s_k + j*s_j + 0] - pv*u_old[k*s_k + j*s_j + 0]
			...
			u_new[k*s_k + j*s_j + Nx] = pa*u[k*s_k + j*s_j + Nx] - pv*u_old[k*s_k + j*s_j + Nx]
	...
	for(int i = 1; i < Nx; i++){
		...
		u_new[0*s_k + 0*s_j + i] = pa*u[0*s_k + 0*s_j + i] - pv*u_old[0*s_k + 0*s_j + i]
		...
		u_new[0*s_k + Ny*s_j + i] = pa*u[0*s_k + Ny*s_j + i] - pv*u_old[0*s_k + Ny*s_j + i]
		...
		u_new[Nz*s_k + 0*s_j + i] = pa*u[Nz*s_k + 0*s_j + i] - pv*u_old[Nz*s_k + 0*s_j + i]
		...
		u_new[Nz*s_k + Ny*s_j + i] = pa*u[Nz*s_k + Ny*s_j + i] - pv*u_old[Nz*s_k + Ny*s_j + i]
	...
	for(int j = 1; j < Ny; j++){
		...
		u_new[0*s_k + j*s_j + 0] = pa*u[0*s_k + j*s_j + 0] - pv*u_old[0*s_k + j*s_j + 0]
		...
		u_new[0*s_k + j*s_j + Nx] = pa*u[0*s_k + j*s_j + Nx] - pv*u_old[0*s_k + j*s_j + Nx]
		...
		u_new[Nz*s_k + j*s_j + 0] = pa*u[Nz*s_k + j*s_j + 0] - pv*u_old[Nz*s_k + j*s_j + 0]
		...
		u_new[Nz*s_k + j*s_j + Nx] = pa*u[Nz*s_k + j*s_j + Nx] - pv*u_old[Nz*s_k + j*s_j + Nx]
	...
	for(int k = 1; k < Nz; k++){
		...
		u_new[k*s_k + 0*s_j + 0] = pa*u[k*s_k + 0*s_j + 0] - pv*u_old[k*s_k + 0*s_j + 0]
		...
		u_new[k*s_k + 0*s_j + Nx] = pa*u[k*s_k + 0*s_j + Nx] - pv*u_old[k*s_k + 0*s_j + Nx]
		...
		u_new[k*s_k + Ny*s_j + 0] = pa*u[k*s_k + Ny*s_j + 0] - pv*u_old[k*s_k + Ny*s_j + 0]
		...
		u_new[k*s_k + Ny*s_j + Nx] = pa*u[k*s_k + Ny*s_j + Nx] - pv*u_old[k*s_k + Ny*s_j + Nx]
	...
	u_new[0*s_k + 0*s_j + 0] = pa*u[0*s_k + 0*s_j + 0] - pv*u_old[0*s_k + 0*s_j + 0]
	...
	u_new[0*s_k + 0*s_j + Nx] = pa*u[0*s_k + 0*s_j + Nx] - pv*u_old[0*s_k + 0*s_j + Nx]
	...
	u_new[0*s_k + Ny*s_j + 0] = pa*u[0*s_k + Ny*s_j + 0] - pv*u_old[0*s_k + Ny*s_j + 0]
	...
	u_new[Nz*s_k + 0*s_j + 0] = pa*u[Nz*s_k + 0*s_j + 0] - pv*u_old[Nz*s_k + 0*s_j + 0]
	...
	u_new[0*s_k + Ny*s_j + Nx] = pa*u[0*s_k + Ny*s_j + Nx] - pv*u_old[0*s_k + Ny*s_j + Nx]
	...
	u_new[Nz*s_k + 0*s_j + Nx] = pa*u[Nz*s_k + 0*s_j + Nx] - pv*u_old[Nz*s_k + 0*s_j + Nx]
	...
	u_new[Nz*s_k + Ny*s_j + 0] = pa*u[Nz*s_k + Ny*s_j + 0] - pv*u_old[Nz*s_k + Ny*s_j + 0]
	...
	u_new[Nz*s_k + Ny*s_j + Nx] = pa*u[Nz*s_k + Ny*s_j + Nx] - pv*u_old[Nz*s_k + Ny*s_j + Nx]
